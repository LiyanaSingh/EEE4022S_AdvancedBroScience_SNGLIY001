...
%% --------------------------- Model Parameters ---------------------------
weight = 74.5; %\cite{puoane2002}
m1 = weight*30.695/1000;   m2 = weight*17.035/1000;   % \cite{drillis1964}        
L1 = 0.31;   L2 = 0.27;              %\cite{Reimann2019ReferenceTW}
...
%% --------------------------- Objective Function ---------------------------
J = 0;
W = diag([1,1,1]);  % control weighting 
for k = 1:N
    uk  = U(:,k); uk1 = U(:,k+1);
    um  = (uk + uk1)/2;
    % Control effort
...
% Compute absolute torque effort (integrated over time)
dt = tgrid(2)-tgrid(1);
abs_effort = sum(abs(U_opt),2)*dt;   % 3x1 vector (tau1, tau2, tau3)
